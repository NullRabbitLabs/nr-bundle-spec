...
A curated, multi-modal dataset of blockchain validator infrastructure under attack and benign workloads. **403 bundles** across **14 chains and network stacks** (Aptos, Bitcoin, Cardano, Cosmos, Dogecoin, Ethereum, IOTA, libp2p, Litecoin, Monero, Solana, Solana (Agave), Sui, XRP), all thirteen families in the taxonomy (twelve attack classes plus `benign`), and **73 attack primitive_ids** — including faithful wire-level reproductions of **51 publicly disclosed CVEs and security advisories** in blockchain node software. Released as the public sample of NullRabbit's bundle corpus, which underpins [NullRabbit Labs'](https://huggingface.co/NullRabbit) ML research on autonomous defence for decentralised networks.
<!-- AUTO:headline:END -->
...
**403 bundles total: 396 attack, 7 benign.** By provenance class: **106 `original`** (NullRabbit's own disclosed findings, each with a published operator advisory), **31 legacy-sample** (the 2026-05-13 Solana/Sui curation), and **266 `public-cve-replication`** (faithful reproductions of already-public CVEs / advisories). The raw `packets.pcap` is dropped from every bundle (`BundleFiles.packets_pcap=False`); no pre-termination pcap variant is retained.
<!-- AUTO:totals:END -->
...
### Public-CVE replications (266 bundles)

These 266 bundles are faithful wire-level reproductions of **51 publicly disclosed CVEs / security advisories** (across 46 attack primitives) in blockchain node software, captured against the vulnerable node so the attack signature is real. Each bundle's `provenance.source_class` is `public-cve-replication` and `provenance.public_source` links the original disclosure.

**Bitcoin (86 bundles, 21 primitives)**
...
**Cosmos (74 bundles, 6 primitives)**
...
**Ethereum (56 bundles, 8 primitives)**
